    ...
    # Ajoutez ces gestionnaires pour la navigation entre les médias
        ...
        return await show_admin_menu(update, context)
            ...
            message_text = (
                ...
                f"Utilisateurs actifs : `{len(active_users)}`\n"
                f"Utilisateurs nettoyés : `{cleaned}`\n\n"
            ...
            for user_id, user_data in list(active_users.items())[:20]:
                ...
                display_name = f"{first_name} {last_name}".strip() or "Nom inconnu"
            ...
            await query.message.edit_text(
                text=message_text,
                reply_markup=InlineKeyboardMarkup(keyboard),
                parse_mode='MarkdownV2'  # Notez le changement ici : MarkdownV2 au lieu de Markdown
            )

    elif query.data == "clean_users":
        # Afficher un message de chargement
        await query.answer("🔄 Nettoyage en cours...")
        await query.message.edit_text(
            "🔄 *Nettoyage de la liste des utilisateurs en cours...*\n"
        ...
        try:
            ...
            text = (
                ...
                f"• Utilisateurs initial : {initial_count}\n"
                f"• Utilisateurs supprimés : {cleaned}\n"
                f"• Utilisateurs restants : {final_count}\n\n"
                "Liste des utilisateurs actifs :\n"
            )
            ...
            active_users = context.bot_data.get('active_users', {})
            for user_id, user_data in list(active_users.items())[:20]:
                username = user_data.get('username', 'Sans @')
                first_name = user_data.get('first_name', '')
                last_name = user_data.get('last_name', '')
                last_seen = user_data.get('last_seen', 'Inconnu')
            ...
            keyboard = [
                [InlineKeyboardButton("🔄 Nettoyer à nouveau", callback_data="clean_users")],
                [InlineKeyboardButton("🔙 Retour", callback_data="admin")]
            ]
            ...
            # Afficher une notification du résultat
            await query.answer(f"✅ {cleaned} utilisateurs inactifs supprimés!")
